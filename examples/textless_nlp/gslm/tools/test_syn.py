...
def main(args, logger):
    ...
    while True:
        ...
        #print("km result",quantized_units.shape)#(846,)
        #print(quantized_units[1])
        quantized_units_str = " ".join(map(str, quantized_units))#len=2533

        tts_input = tts_dataset.get_tensor(quantized_units_str)
        #print(tts_input.shape)#[212]
        mel, aud, aud_dn, has_eos = synthesize_audio(
            tacotron_model,
            waveglow,
            denoiser,
            tts_input.unsqueeze(0),#[1,212]
            strength=args.denoiser_strength,
        )#shape(aud)=shape(aud_dn)=[1,256*len(mel)]
        print(aud_dn[0])
